...
This last development requires a distinction between several uses of the term weight.
...
labor-supply outcome, and a logged monetary outcome. Together, the applications reveal
whether the diagnostic conclusions persist when the source of instrument variation, the
covariate structure, and the outcome scale change.
...
covariates. Treatment is endogenous when treatment status is related to
unobserved determinants of the potential outcomes, so a direct comparison
of treated and untreated observations need not identify a causal effect.
Instrumental-variable methods instead use variation in treatment induced by
$Z_i$. Under heterogeneous treatment effects, the resulting estimand depends
...
Abadie's representation formalizes this conditional-IV approach. Following
\citet{abadie2003semiparametric}, let $p(X_i)=P(Z_i=1\mid X_i)$ denote
the conditional instrument propensity score. The identification result relies
...
\begin{enumerate}
    ...
    \]
\end{enumerate}
Condition (i) requires the instrument to be as good as randomly assigned within
covariate strata. Condition (ii) rules out a direct effect of the instrument on
the outcome. Condition (iii) combines overlap with a positive conditional first
stage, while condition (iv) rules out defiers. Under conditions (i) and (iv) for almost every covariate value \(x\),
\begin{align}
...
\end{align}
Thus, the conditional first stage equals the conditional complier share.
Always- and never-takers contribute zero, whereas each complier contributes
one. Equation~\eqref{eq:conditional_complier_share} clarifies which population
supplies the identifying treatment variation, but individual compliance status
remains latent. Abadie's key result is that moments of this latent group,
including the potential-outcome means in
Equation~\eqref{eq:late_definition}, can nevertheless be recovered from
observed data.\\
For this purpose, define the three kappa weights
...
status is not observed. By choosing \(g\), \(g_0\), and \(g_1\) appropriately, moments of the
...
\end{equation}
Applying parts (b) and (c) to the LATE defined in
Equation~\eqref{eq:late_definition}, set $g_0(Y_i(0),X_i)=Y_i(0)$ and
...
weighting estimators of $\tau_{\mathrm{LATE}}$ developed in the next section.
...
\begin{equation}
    ...
    \label{eq:tau_a10}
\end{equation}
Here each component has its own normalization denominator. 
