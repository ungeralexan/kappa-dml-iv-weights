...
years of schooling and \(27.1\%\) completed at least sixteen years. The
...
specification. The corresponding squared HC1-robust \(t\)-statistics are
\(12.5\) and \(9.0\). At the sixteen-year margin, the adjusted first stages
...
Estimates for the sixteen-year treatment are generally larger and less
...
information. However, the two treatment definitions describe different
schooling margins and therefore different causal parameters. The larger
sixteen-year coefficients should consequently not be interpreted as evidence
...
untuned and tuned comparison. The following subsection
examines whether these point-estimate patterns remain stable when the outcome
is translated.
...
important source of numerical sensitivity. The rerun exercise does not,
however, establish whether this difference is caused by the number of
covariates itself or by their interaction with nuisance estimation.\\
Tuned XGBoost is classified as non-invariant in all eight comparisons,
including those based on the Kitagawa specification. Its sensitivity therefore
cannot be explained solely by the richer Card conditioning set. The pattern
...
respective normalisation class. An estimate is classified as unchanged when
the absolute rerun change does not exceed \(10^{-4}\). Bold entries exceed
this threshold. Values displayed as zero are zero up to the reported
precision.
\end{tablenotes}
\end{threeparttable}
\end{table}
