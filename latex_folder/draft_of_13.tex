...
Appendix Table~\ref{tab:vietnam_dml} reports the two DML estimates together with the
...
Appendix Table~\ref{tab:vietnam_dml}; the other fits are treated as sensitivity
...
The replication results show why translation invariance is an important
property in this application. Measuring wages in cents instead of dollars
adds \(\log(100)\) to the outcome but does not change the underlying economic
quantity. The
normalised kappa estimators in Section~\ref{subsec:vietnam_estimates} remain
unchanged, while the unnormalised estimators move under the linear and cubic
age specifications.
I apply the same outcome translation to the DML estimators. For this purpose,
each complete estimation procedure is fitted once to log wages in dollars and
once to the translated outcome. The nuisance functions are re-estimated using
the same cross-fitting design. Honest-forest tuning is repeated internally,
and the XGBoost specifications repeat the nested tuning procedure within the
...
\end{threeparttable}
\end{table}
Table~\ref{tab:vietnam_translation} first restates the kappa benchmark for the
two specifications used in the DML analysis. The three normalised estimators
are unchanged. By contrast, the cubic unnormalised estimates increase by
between \(0.213\) and \(0.224\) log points. Under saturated age indicators,
all six kappa estimators coincide. As discussed above, this is a feature of
the age-cell specification in this sample and not a general invariance result
for the unnormalised estimators.
The honest-forest, OLS/logit, and \texttt{ranger} estimates are also
numerically unchanged after complete re-estimation. The only nonzero
honest-forest difference is \(0.000018\) for saturated Wald-AIPW, which is
negligible at the reported precision. The tuned XGBoost estimates show small
but visible changes. The largest is \(0.002917\) for cubic PLR-IV. These
changes are minor relative to both the estimated effects and their standard
errors, so the substantive conclusion of the Vietnam application is stable.
At the same time, exact equality is not obtained for the tuned XGBoost
procedures.
This qualification is consistent with the discussion in
\citet{knaus2024treatment}. Boosted-tree learners do not generally provide
the affine-smoother structure used to establish the corresponding
normalisation results. The XGBoost findings reported here refer to the
implemented learner and nested tuning procedure. The next section examines
the realised outcome weights to distinguish the reproduction of the point
estimate from the normalisation and concentration of the recovered weights.
...
\subsection{Normalization and Outcome-Weight Diagnostics}
\label{subsec:vietnam_weights}
Section~\ref{subsec:vietnam_translation} documents how the implemented
estimators behave when the outcome is translated and the complete procedure
is re-estimated. I now examine the realised outcome weights to determine
whether they reproduce the reported estimate, how they are normalised, and
whether they place substantial weight on a small part of the sample.

\begin{sidewaystable}[p]
\centering
\caption{Outcome-weight representation, normalisation, and concentration}
...
& $\max_i|\omega_i|$ \\
\midrule
\multicolumn{7}{l}{\textit{Panel A: Cubic age specification}} \\[2pt]
PLR-IV, tuned forest
& $6.88\times10^{-15}$ & 0.000000 & 0.994/0.994
& 0.963/0.921 & 43.5/46.6 & 0.012494 \\
Wald-AIPW, tuned forest
& $8.69\times10^{-15}$ & 0.000000 & 1.000/1.000
& 0.952/0.910 & 43.4/46.7 & 0.017627 \\
PLR-IV, tuned XGBoost
& $1.18\times10^{-7}$ & 0.000627 & 0.993/0.993
& 0.962/0.917 & 43.5/46.6 & 0.013060 \\
Wald-AIPW, tuned XGBoost
& $3.99\times10^{-9}$ & 0.000027 & 1.001/1.001
& 0.951/0.907 & 43.3/46.7 & 0.019535 \\[2pt]
$\hat{\tau}_{u}^{cb}$
& 0 & 0.000000 & 1.000/1.000
...
$\hat{\tau}_{a}$
& 0 & 0.048243 & 1.041/0.993
& 0.954/0.898 & 43.5/46.9 & 0.028869 \\
$\hat{\tau}_{t}$
& 0 & 0.046342 & 1.000/0.954
& 0.954/0.898 & 43.5/46.9 & 0.027731 \\
$\hat{\tau}_{a,0}$
& 0 & 0.048594 & 1.049/1.000
& 0.954/0.898 & 43.5/46.9 & 0.029078 \\[4pt]

\multicolumn{7}{l}{\textit{Panel B: Saturated age specification}} \\[2pt]
PLR-IV, tuned forest
& $5.33\times10^{-15}$ & 0.000000 & 0.999/0.999
& 0.961/0.917 & 43.5/46.6 & 0.013128 \\
Wald-AIPW, tuned forest
& $5.97\times10^{-15}$ & 0.000000 & 1.005/1.005
& 0.951/0.908 & 43.4/46.7 & 0.016866 \\
PLR-IV, tuned XGBoost
& $1.17\times10^{-7}$ & 0.000019 & 1.011/1.011
& 0.961/0.920 & 43.4/46.6 & 0.012257 \\
Wald-AIPW, tuned XGBoost
& $3.46\times10^{-8}$ & $-0.000075$ & 1.005/1.005
& 0.950/0.911 & 43.2/46.6 & 0.016570 \\[2pt]
$\hat{\tau}_{u}^{cb}$
& 0 & 0.000000 & 1.000/1.000
...
The representation gap is the absolute difference between the weighted
outcome sum and the reported estimate. The XGBoost rows use reconstructed
weight vectors. Modified ESS is divided by the observed number of treated or
control observations. Opposite-sign mass is reported in percent. The forest
and XGBoost rows use the tuned specifications.
\end{tablenotes}
\end{threeparttable}
\end{sidewaystable}
The representation gap is reported before the weight statistics because the
diagnostics are exact only when the recovered weights reproduce the estimate.
The kappa weights do so by construction. The tuned-forest gaps are below
\(10^{-14}\), which confirms the numerical representation. The reconstructed
XGBoost gaps are also small, but they are larger and do not satisfy the exact
numerical check used in the implementation. Their weight statistics are
therefore interpreted as learner-specific diagnostics rather than as an exact
decomposition of the reported estimate.\\
The cubic kappa results provide the clearest normalisation comparison. The
three normalised estimators have total weight zero and treated and control
masses equal to one. The unnormalised estimators instead have total weights
between \(0.0463\) and \(0.0486\). Their separate masses also show which part
of the contrast remains unnormalised. For \(\hat{\tau}_{t}\), the treated mass
equals one while the control mass is \(0.954\). For
\(\hat{\tau}_{a,0}\), the control mass equals one while the treated mass is
\(1.049\). These differences account for the translation sensitivity in
Table~\ref{tab:vietnam_translation}.\\
The concentration statistics add a separate finding. Despite their different
normalisation, the cubic kappa estimators have almost identical modified ESS
ratios, opposite-sign masses, and maximum absolute weights. Their modified ESS
ratios are approximately \(0.95\) among treated observations and \(0.90\)
among controls, while all maximum weights remain below \(0.030\). The outcome
translation problem is therefore linked to the weight masses rather than to a
small effective sample or a few dominant observations.\\
The tuned-forest weights show a similar concentration pattern and have total
weight zero in both age specifications. Their treated and control masses are
also close to one. The reconstructed XGBoost weights have small total masses
and comparable concentration statistics. Since these reconstructed weights
do not reproduce the estimates exactly, their near-normalisation should not
be interpreted as an exact finite-sample property.\\
Under saturated age indicators, all six kappa estimators have the same weight
statistics. This is consistent with their common point estimate and their
unchanged rerun results in this sample. Appendix
Table~\ref{tab:vietnam_weights_learners_appendix} reports the corresponding
diagnostics for OLS/logit and \texttt{ranger}. These learner specifications
lead to the same broad conclusion. The weights are not strongly concentrated,
and the differences across learners are small relative to the distinction
between normalised and unnormalised kappa weights under cubic age.
...
\section{DML and Benchmark Estimates for the Vietnam Application}
\begin{table}[htbp]
\centering
...
\begin{sidewaystable}[p]
\centering
\caption{Learner-specific outcome-weight diagnostics: Vietnam draft lottery}
\label{tab:vietnam_weights_learners_appendix}
\footnotesize
\begin{threeparttable}
\begin{tabular}{l r r c c c r}
\toprule
...
\multicolumn{7}{l}{\textit{Panel A: Cubic age specification}} \\[2pt]
PLR-IV, OLS
& $3.71\times10^{-9}$ & 0.000000 & 1.000/1.000
& 0.966/0.924 & 43.6/46.7 & 0.014577 \\
Wald-AIPW, OLS/logit
& $1.20\times10^{-9}$ & 0.000000 & 0.991/0.991
& 0.955/0.908 & 43.5/46.8 & 0.026926 \\
PLR-IV, \texttt{ranger}
& $2.22\times10^{-16}$ & 0.000000 & 0.999/0.999
& 0.960/0.916 & 43.5/46.6 & 0.013538 \\
Wald-AIPW, \texttt{ranger}
& $4.11\times10^{-15}$ & 0.000000 & 0.999/0.999
& 0.948/0.903 & 43.4/46.7 & 0.021647 \\[4pt]

\multicolumn{7}{l}{\textit{Panel B: Saturated age specification}} \\[2pt]
PLR-IV, OLS
& $5.72\times10^{-15}$ & 0.000000 & 1.000/1.000
& 0.960/0.916 & 43.5/46.6 & 0.013426 \\
Wald-AIPW, OLS/logit
& $3.37\times10^{-14}$ & 0.000000 & 1.000/1.000
& 0.948/0.903 & 43.4/46.7 & 0.021773 \\
PLR-IV, \texttt{ranger}
& $2.11\times10^{-15}$ & 0.000000 & 1.003/1.003
& 0.964/0.924 & 43.3/46.5 & 0.011886 \\
Wald-AIPW, \texttt{ranger}
& $3.77\times10^{-15}$ & 0.000000 & 1.005/1.005
& 0.951/0.914 & 43.1/46.6 & 0.016211 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\footnotesize
\item \textit{Notes:} T/C denotes treated and sign-oriented control values.
The representation gap is the absolute difference between the weighted
outcome sum and the reported estimate. Modified ESS is divided by the
observed number of treated or control observations. Opposite-sign mass is
reported in percent.
\end{tablenotes}
\end{threeparttable}
\end{sidewaystable}
