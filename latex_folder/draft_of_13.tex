...
All outcome nuisance functions are re-estimated. Honest-forest tuning is
repeated internally, while the XGBoost specifications repeat nested tuning
...
This section examines whether the estimated outcome weights reproduce the reported point estimates, how the weights are normalised, and whether the estimates depend strongly on a small number of observations. The main discussion focuses on the cubic age specification because it produces the clearest differences across estimators and nuisance learners. Under saturated age adjustment, the kappa estimators have identical weight diagnostics, so a separate detailed discussion would largely repeat the same points. The complete results for both age specifications are reported in Appendix Table~\ref{tab:vietnam_weights_full_appendix}.
...
& $\max_i|\omega_i|$ \\
\midrule
\multicolumn{7}{l}{\textit{Honest-forest estimators}} \\[2pt]
PLR-IV
& $6.88\times10^{-15}$ & 0.0000000 & 0.9942/0.9942
& 43.5/46.6 & 0.963/0.921 & 0.012494 \\
Wald-AIPW
& $8.69\times10^{-15}$ & 0.0000000 & 0.9998/0.9998
& 43.4/46.7 & 0.952/0.910 & 0.017627 \\[4pt]

\multicolumn{7}{l}{\textit{Linear and logistic nuisance models}} \\[2pt]
PLR-IV, OLS
& $3.71\times10^{-9}$ & 0.0000000 & 1.0000/1.0000
& 43.6/46.7 & 0.966/0.924 & 0.014577 \\
Wald-AIPW, OLS/logit
& $1.20\times10^{-9}$ & 0.0000000 & 0.9915/0.9915
& 43.5/46.8 & 0.955/0.908 & 0.026926 \\[4pt]

\multicolumn{7}{l}{\textit{Reconstructed XGBoost weights}} \\[2pt]
PLR-IV, tuned XGBoost
& $1.18\times10^{-7}$ & 0.0006273 & 0.9932/0.9925
& 43.5/46.6 & 0.962/0.917 & 0.013060 \\
Wald-AIPW, tuned XGBoost
& $3.99\times10^{-9}$ & 0.0000268 & 1.0013/1.0013
& 43.3/46.7 & 0.951/0.907 & 0.019535 \\[4pt]

\multicolumn{7}{l}{\textit{Kappa estimators}} \\[2pt]
$\hat{\tau}_{u}^{cb}$
& 0 & 0.0000000 & 1.0000/1.0000
& 43.5/46.8 & 0.958/0.908 & 0.025384 \\
$\hat{\tau}_{u}^{ml}$
& 0 & 0.0000000 & 1.0000/1.0000
& 43.6/46.8 & 0.954/0.899 & 0.028108 \\
$\hat{\tau}_{a,10}$
& 0 & 0.0000000 & 1.0000/1.0000
& 43.5/46.9 & 0.954/0.898 & 0.029078 \\
$\hat{\tau}_{a}$
& 0 & 0.0482434 & 1.0410/0.9928
& 43.5/46.9 & 0.954/0.898 & 0.028869 \\
$\hat{\tau}_{t}$
& 0 & 0.0463418 & 1.0000/0.9537
& 43.5/46.9 & 0.954/0.898 & 0.027731 \\
$\hat{\tau}_{a,0}$
& 0 & 0.0485937 & 1.0486/1.0000
& 43.5/46.9 & 0.954/0.898 & 0.029078 \\
\bottomrule
\end{tabular}%
}
\begin{tablenotes}
\footnotesize
\item \textit{Notes:} T/C denotes treated and sign-oriented control values. The representation gap is the absolute difference between the weighted outcome sum and the reported estimate. Opposite-sign mass is the percentage of absolute weight mass that has the opposite sign from the conventional treated-positive and control-negative orientation. It is a measure of cancellation and does not by itself indicate that an estimator is invalid. The arm-specific modified ESS is divided by the observed number of treated or control observations. The XGBoost rows are based on reconstructed weight vectors. The complete cubic and saturated results are reported in Appendix Table~\ref{tab:vietnam_weights_full_appendix}.
\end{tablenotes}
\end{threeparttable}
\end{table}

The representation gaps first establish whether the estimated weights provide an accurate weighted representation of the reported estimator. The kappa weights reproduce their estimates by construction. The honest-forest gaps are below \(10^{-14}\). The gaps are also small for the linear and logistic nuisance models. The reconstructed XGBoost weights are less exact. In particular, the PLR-IV gap is \(1.18\times10^{-7}\), which fails the strict numerical equality check used in the implementation. \\
The cubic kappa results provide the clearest comparison of the different normalisation classes introduced by \citet[Table~4]{knaus2024treatment}. The weights of \(\hat{\tau}_{u}^{cb}\), \(\hat{\tau}_{u}^{ml}\), and \(\hat{\tau}_{a,10}\) sum to zero, while the treated and sign-oriented control masses both equal one. These estimators are therefore fully normalised. By contrast, \(\hat{\tau}_{a}\) is fully unnormalised because neither group mass satisfies the corresponding normalisation restriction. The weights of \(\hat{\tau}_{t}\) are untreated-unnormalised because the treated mass equals one while the sign-oriented control mass is only \(0.954\). The reverse pattern holds for \(\hat{\tau}_{a,0}\), which is treated-unnormalised because its control mass equals one while the treated mass is \(1.049\). These results correspond to the classification of the kappa estimators in \citet[Appendix~A.4]{knaus2024treatment}.\\
The DML results also follow the theoretical distinction in \citet[Table~5]{knaus2024treatment}. When the nuisance learner is an affine smoother, Condition~3 is sufficient for the PLR-IV and Wald-AIPW weights to sum to zero. Full normalisation additionally requires the treatment prediction to be constructed using the same smoothing structure as the outcome prediction, as formalised by Conditions~5a and 5b. The honest-forest PLR-IV and Wald-AIPW weights sum to zero, but their group masses are \(0.994\) and \(0.9998\), respectively. They are therefore scale-normalised and close to, but not exactly, fully normalised.\\
The OLS implementation of PLR-IV displays the fully normalised pattern, with both group masses equal to one. This is consistent with Condition~5a when the same linear projection structure is used for the treatment and outcome nuisance functions. In contrast, Wald-AIPW combines linear outcome regressions with a logistic treatment model. Its two group masses are equal but amount to approximately \(0.991\), so the weights are scale-normalised rather than fully normalised. This difference illustrates why full normalisation depends on the complete nuisance-function design rather than only on the choice of estimator.\\
For XGBoost, the affine-smoother condition is not generally available. The small nonzero total weight sums are therefore consistent with the absence of the normalisation guarantee in Table~5. Table~6 of \citet{knaus2024treatment} illustrates the same distinction for random-forest and XGBoost implementations of PLR-IV and Wald-AIPW. \\
The concentration diagnostics lead to a separate finding. Across the estimators in Table~\ref{tab:vietnam_weights}, the arm-specific modified ESS ratios range from approximately \(0.90\) to \(0.97\), while all maximum absolute weights remain below \(0.03\). The corresponding maximum weights for the honest-forest specifications are \(0.0125\) for PLR-IV and \(0.0176\) for Wald-AIPW. These results provide little evidence that the estimates are dominated by a small number of observations.\\
At the same time, the weights involve substantial cancellation. Between \(43.3\%\) and \(43.6\%\) of the absolute treated weight mass and between \(46.6\%\) and \(46.9\%\) of the absolute control weight mass have the opposite sign from the conventional orientation. High modified ESS ratios and small maximum weights therefore do not imply that the weighting structure is simple. Instead, the estimates combine many positive and negative contributions that offset one another. The similarity of these diagnostics across the normalisation classes also shows that normalisation and weight concentration capture different properties of an estimator.\\
The `ranger` results, reported in Appendix Table~\ref{tab:vietnam_weights_learners_appendix}, display the same broad pattern as the honest-forest results. Their weights sum to zero, their treated and control masses are equal and close to one, and their concentration measures are similar. Under saturated age adjustment, all six kappa estimators have exactly the same weight sums, group masses, modified ESS ratios, opposite-sign masses, and maximum absolute weights. The saturated specification therefore adds little further differentiation between the kappa estimators, but it provides a useful confirmation that their weighting structures coincide in this specification.
...
Appendix Figures~\ref{fig:vietnam_love_cubic} and
