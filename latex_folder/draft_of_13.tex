...
control observations. The forest and XGBoost rows use the tuned
specifications. The complete cubic and saturated results are reported in
Appendix Table~\ref{tab:vietnam_weights_full_appendix}.
\end{tablenotes}
\end{threeparttable}
\end{table}
The representation gap is reported first because the remaining statistics
describe an exact outcome-weight representation only when
\(\omega'Y=\hat{\tau}\). The kappa weights satisfy this equality by
construction, and the tuned-forest gaps are below \(10^{-14}\). The gaps for
the reconstructed XGBoost weights are small but fail the exact numerical check
used in the implementation. Their statistics are therefore treated as
diagnostics of the reconstructed learner weights rather than as exact
decompositions of the reported estimates.

The cubic kappa results give the clearest normalisation comparison. For
\(\hat{\tau}_{u}^{cb}\) and \(\hat{\tau}_{u}^{ml}\), total weight is zero and
the treated and control masses both equal one. The incompletely normalised
estimators instead have total weights between \(0.0463\) and \(0.0486\).
For \(\hat{\tau}_{t}\), the treated mass equals one while the control mass is
\(0.954\). For \(\hat{\tau}_{a,0}\), the control mass equals one while the
treated mass is \(1.049\). These mass differences correspond to the
translation sensitivity in Table~\ref{tab:vietnam_translation}.

The concentration measures lead to a separate conclusion. The cubic kappa
estimators have modified ESS ratios of about \(0.95\) among treated
observations and \(0.90\) among controls. Their maximum absolute weights
remain below \(0.030\), despite their different normalisation. The sensitivity
to the outcome translation is therefore associated with the weight masses,
not with a small effective sample or a few dominant observations. The tuned
forest and reconstructed XGBoost weights show a similar degree of
concentration.

Under saturated age adjustment, all six kappa estimators have the same weight
statistics. This agrees with their common point estimate and unchanged rerun
results in this sample. The complete cubic and saturated comparison is
reported in Appendix Table~\ref{tab:vietnam_weights_full_appendix}. Appendix
Table~\ref{tab:vietnam_weights_learners_appendix} adds the OLS/logit and
\texttt{ranger} specifications.

\subsection{Covariate Balance}
\label{subsec:vietnam_balance}
The preceding diagnostics concern the representation, normalisation, and
concentration of the outcome weights. Covariate balance is examined
separately. Appendix Figures~\ref{fig:vietnam_love_cubic} and
\ref{fig:vietnam_love_saturated} compare the tuned forest and XGBoost weights
with the CBPS-based and maximum-likelihood kappa weights.\\
Under cubic age adjustment, the unadjusted standardised differences in age,
age squared, and age cubed are between \(0.52\) and \(0.56\). Weighting reduces
these differences substantially, but the degree of balance is not identical
across estimators. The maximum adjusted difference is effectively zero for
\(\hat{\tau}_{u}^{cb}\). It equals \(0.099\) for tuned-forest PLR-IV and
\(0.109\) for \(\hat{\tau}_{u}^{ml}\), while the two XGBoost specifications
and tuned-forest Wald-AIPW remain below \(0.03\). The MLE-based kappa value is
therefore slightly above the \(0.1\) reference line. The contrast between the
two kappa estimators reflects their different propensity-score fits. CBPS
directly imposes balancing conditions, whereas maximum likelihood does not.\\
With saturated age indicators, the unadjusted absolute differences are all
below \(0.1\). Both kappa estimators balance the included age indicators up to
numerical precision. The maximum adjusted difference is \(0.002\) for
tuned-forest PLR-IV, \(0.029\) for tuned-forest Wald-AIPW, \(0.020\) for
XGBoost PLR-IV, and \(0.045\) for XGBoost Wald-AIPW. These results show that
the flexible learner weights reduce the maximum cohort difference, but they
do not move every individual age indicator closer to zero. They also do not
reproduce the exact balance obtained by the saturated kappa specification.
The XGBoost panels remain subject to the representation qualification above.
Balance is a property of the fitted weighted comparison. It does not establish
the IV identifying assumptions and is distinct from the normalisation results
above.

\subsection{Summary of the Vietnam Application}
\label{subsec:vietnam_summary}
The Vietnam application links the reproduction exercise to the DML and
outcome-weight analysis. The cents and dollars comparison reproduces the main
pattern documented by \citet{sloczynski2025abadie}. The normalised kappa
estimators are unchanged after the outcome translation, while the
incompletely normalised estimators change under cubic age adjustment. Under
saturated age indicators, all six kappa estimators coincide in this sample.\\
The DML estimates are also stable across the considered learners and tuning
choices relative to their statistical uncertainty. Complete re-estimation
after the outcome translation leaves the honest-forest, OLS/logit, and
\texttt{ranger} estimates numerically unchanged. The tuned XGBoost estimates
move slightly, with a largest difference of \(0.0029\). The reconstructed
XGBoost weights do not satisfy the exact representation check, so their
weight diagnostics are interpreted with this qualification.\\
The outcome-weight analysis explains the kappa result more directly. Under
cubic age adjustment, translation sensitivity occurs when the treated and
control weight masses are not both equal to one. At the same time, the
effective sample sizes and maximum weights remain similar across the kappa
estimators. Normalisation and weight concentration therefore describe
different features of the estimators. The balance results add a further
distinction. All selected estimators reduce the age differences, but exact
balance depends on the propensity-score specification and is not implied by
translation invariance.\\
The following Card application moves from the draft lottery to an
observational instrument with a richer covariate set. It therefore provides a
more demanding setting for the same estimator and weight diagnostics.
...
\newpage
\section{Vietnam Outcome-Weight Diagnostics}
\begin{sidewaystable}[p]
\centering
\caption{Complete outcome-weight diagnostics for the Vietnam application}
...
\clearpage
\section{Vietnam Covariate-Balance Diagnostics}
\begin{figure}[p]
\centering
