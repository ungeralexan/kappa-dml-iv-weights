...
Here maybe a nice introduction or directly the key point.
The key point is that economically equivalent representations of an outcome should lead to the same estimated effect. For logged monetary outcomes, changing the underlying unit adds a constant to every logged outcome. 
An estimator that changes under this transformation depends partly on an arbitrary measurement convention.
Moroever mentioning the notions of translation invariance and scale equivariance are the main designs later in this thesis but also related to \citet{sloczynski2025abadie} and here which estimates are used and the ones that are based on abbadie should include his 2003 framework and the emphasis on that maybe rather mentioning that genau hier this thesis connects to that.
Here then mentioning that we follow the framework of \citet{sloczynski2025abadie} as we as well tackle the Instrumental variable framework and here for that use the given estimators and the new double machine learning
estimators.
Both can be connected via outcome weights which follows knaus closely and opens a framewrok that can give us a deeper insight into how an estimator actually uses the sample and digs also deeper into how balancing works. Next to that we of course follow the importnat notion of \citet{sloczynski2025abadie} which is normalization and will extend it and also chekcing double machine learning estumators connecting it to a freamrwork of knaus tht analyticallly showded how normalization in the weighst can be shown.
...
This is a very important section that on the one hand should establish this chronological order that moves from iv and unnormalized estimates to diagnostics in combination with normalization and what has been done.
The Instrumental variable framewrok that is very specific and not so much discovered and the notion of blandhole at all what happens once we condition and many estimates.\\


The problem how recent literature uses unnomrlaized weights which papers (problem is that is what is already mentioned by \citet{sloczynski2025abadie}\\
\citet{abadie2003semiparametric} establishes those weights even though weights are not often used in the Instrumental varibale framework \citet{sloczynski2025abadie} out of this framework weighst are constructed\\
Estimator multiplicity tehre are certain estimates as the one of Fröhlich or Tann and Uysal but only a few are actually normalized and the rest either has normalizaion results or not (Here we introduce the normalization for the very first time)\\
When it comes to normalization broader noramliztaiion reaferences next to \citet{sloczynski2025abadie} could be mentioned with Khan and Ugander on adaptive normalization if you want a broader normalization reference,  but taken into account I have zero idea what their paper is about. 
\\
The machine learning framework and that their are estimates as well based on that and the modern technology that actually can capture nonlinearities, here maybe some citing would not be bad; Here in connection that machine learning nowadays can also be used for causal inference in \citet{chernozhukov2018double}.
\\ This machine learning framework can be used for for estimates such as PLR IV or wald aipw in the double machine learning framework using generalized random forest by \citet{athey2019generalized}.
Next to those techniques such as ranger needs to be ceited as well breiman, xgboost such as breiman again and linear regression could be used. To estimate nuisance parameters more flexible. 
\\ This has been used both already in the framework of knaus with the purpose of looking deeper in the weights and establishing things such as covariate balancing and having a deeper look into the normalization of treated and untreated weights and so on.
...
Connecting the whole idea to what I do and being way more specific of what I am doing. 
What they have been doing in the \citet{sloczynski2025abadie} paper but that there are IV estimators nowadays that use the novel design of \citet{chernozhukov2018double}
which is double machine learning and can be used. 
Combine it with the knaus framework and the normaliztation idea of to make a real contribution. That for how I imagine that it is time to mention it foming my general research idea and my three sub research ideas especifically. 
Mention my research question in connection to that and the three sub research question and here especially they all should be more applied to what I am doing becaise back then when I formulated them they were a bit to generalized. 
For this framework and to answer those we will be going thorugh 3 fundamental designs of the instrumental variable framework to test that that are based on three of the greatest IV researches and are combined in  this manner by the \cite{sloczynski2025abadie} paper.
For those we will have  certain diagnostics list them that we will use and hence get more then a glimpse which observations are weighted in a way and get more how this certain estimators weights the sample and as well balances the samples and maybe guarantees a fair comparison or not. 
Then how do we assess normalization especially for the double machine learning estimates and hence tackle translation invarinace and scale invariance. 
Mentioning how we implemnet that like with which computational framework we create comapred ot the knau outcome weights package in R and the kappalate framework in stata of \citet{sloczynski2025abadie} and how we combine those two in this one framework of mine and use the other packages to unterstützen.

Mention in the next sections how we are gonna proceed like every academic paper does. 
