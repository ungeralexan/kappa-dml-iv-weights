...
# Chattopadhyay & Zubizarreta (2023, Section 5.4) recommend computing their
# absolute-weight ESS separately by treatment group.  For controls we orient
# the weights by -1, so positive oriented weights have the conventional sign in
# both groups.  Wrong_sign_mass is the share of absolute mass, rather than the
# share of observations, carried by weights with the wrong oriented sign.
...
# Presentation wrapper: T/C pairs keep the empirical tables compact while the
# unrounded numeric columns remain available from outcome_weight_diagnostics().
...
# Conventional Kish ESS for a positive design-stage weighted sample.  The
# general (sum w)^2/sum(w^2) form is used, so callers need not pre-normalize.
...
# ==============================================================================
# SE analogue of get_dml_coef() (§E). Uses the fitted object's direct `se`
# accessor when available, with a quiet summary-table fallback that tolerates
# either column name used across DoubleML versions ("std err" or "Std. Error").
# Returns NA_real_ if neither route supplies a finite scalar.
#
